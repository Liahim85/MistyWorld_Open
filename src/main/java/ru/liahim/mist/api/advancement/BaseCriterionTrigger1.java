 ...
 */
public abstract class BaseCriterionTrigger1<D, T extends ICriterionInstanceTestable1<D>> implements ICriterionTrigger<T> {

	private final Map<PlayerAdvancements, Listeners<D, T>> listeners = Maps
			.<PlayerAdvancements, BaseCriterionTrigger1.Listeners<D, T>> newHashMap();
	private final ResourceLocation id;

	protected BaseCriterionTrigger1(ResourceLocation id) {
		this.id = id;
	}
	...
	public void addListener(PlayerAdvancements playerAdvancementsIn, Listener<T> listener) {
		BaseCriterionTrigger1.Listeners<D, T> listeners = this.listeners.get(playerAdvancementsIn);

		if (listeners == null) {
			listeners = new BaseCriterionTrigger1.Listeners<>(playerAdvancementsIn);
	...
	public void trigger(EntityPlayerMP player, D criterionData) {
		BaseCriterionTrigger1.Listeners<D, T> listeners = this.listeners.get(player.getAdvancements());
	...
	static class Listeners<D, T extends ICriterionInstanceTestable1<D>> {

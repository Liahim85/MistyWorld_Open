...
/**@author Liahim*/
public class MistFarmland extends MistDirt implements IFarmland {

	protected static final AxisAlignedBB FARMLAND_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.9375D, 1.0D);

	public MistFarmland(float hardness, int waterPerm) {
		super(Material.GROUND, hardness, waterPerm);
		this.setDefaultState(this.blockState.getBaseState().withProperty(HUMUS, 0).withProperty(WET, true).withProperty(MULCH, false));
	...
	public boolean doIfWet(World world, BlockPos pos, IBlockState state, Vector2f waterType, boolean fog, Random rand) {
		if (state.getValue(MULCH)) {
			int humus = state.getValue(HUMUS);
			if (humus < getHumSize() - 1 && rand.nextInt(humus * 25 + 50) == 0) {
				return world.setBlockState(pos, state.withProperty(HUMUS, humus + 1).withProperty(MULCH, false));
			}
		}
		return super.doIfWet(world, pos, state, waterType, fog, rand);
	}
	...
	public void onFallenUpon(World world, BlockPos pos, Entity entity, float fallDistance) {
		if (!world.isRemote && world.rand.nextFloat() < fallDistance - 0.5F && entity instanceof EntityLivingBase &&
			...
			if (!state.getValue(MULCH)) world.setBlockState(pos, this.getSoilBlock().getDefaultState().withProperty(HUMUS, state.getValue(HUMUS)).withProperty(WET, state.getValue(WET)));
		}
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
		...
		if (change) world.setBlockState(pos, this.getSoilBlock().getDefaultState().withProperty(HUMUS, state.getValue(HUMUS)).withProperty(WET, state.getValue(WET)));
	...
	public void neighborChanged(IBlockState state, World world, BlockPos pos, Block block, BlockPos fromPos) {
		...
		if (upMat.isSolid()) {
			if (!state.getValue(MULCH)) {
				world.setBlockState(pos, this.getSoilBlock().getDefaultState().withProperty(HUMUS, state.getValue(HUMUS)).withProperty(WET, state.getValue(WET)));
			}
	...
	public boolean isFertile(World world, BlockPos pos) {
		IBlockState state = world.getBlockState(pos);
		return state.getValue(WET) && state.getValue(HUMUS) > 0;
	}

	@Override
	public int getMetaFromState(IBlockState state) {
		return state.getValue(HUMUS) + (state.getValue(WET) ? 0 : getHumSize()) + (state.getValue(MULCH) ? getHumSize() * 2 : 0);
	}

	@Override
	public IBlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(HUMUS, meta % getHumSize()).withProperty(WET, meta % (getHumSize() * 2) < getHumSize()).withProperty(MULCH, meta >= getHumSize() * 2);
	}

	@Override
	protected BlockStateContainer createBlockState() {
		return new BlockStateContainer(this, new IProperty[] { HUMUS, MULCH, WET });
	}

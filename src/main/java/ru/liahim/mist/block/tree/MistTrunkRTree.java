...
public class MistTrunkRTree extends MistTreeTrunk {

	public MistTrunkRTree() {
		super(5.0F, 1, 2, false, true, true, (MistTreeLeaves)MistBlocks.R_TREE_LEAVES, 1, 0, 6, new int[] {1, 1, 1, 1});
	...
	public void generateTree(World world, BlockPos pos, Random rand, boolean checkSnow) {
		if (!world.isRemote) {
			...
			if (checkState.getBlock() instanceof MistSoil && checkEnvironment(world, pos, false) && world.canBlockSeeSky(pos)) {
				...
				if (size == 4) SoilHelper.setSoil(world, pos.down(), ((MistSoil)checkState.getBlock()).getSoilBlock().getDefaultState(), 2, true, Mist.FLAG);
				boolean node;
							...
							if (rand.nextInt(3) > 0) {
								...
								if (checkEnvironment(world, checkPos, false)) {
									...
									if ((cropTop && i == maxTreeHight - 1) ||
											checkState.getBlock() == this ? getDir(checkState) != dir : true) {
					...
					for (int y = minTrunckLength; y <= maxTreeHight; ++y) {
						for (int z = -3; z < 4; ++z) {
							...
							if (checkState.getBlock() == this.leaves) {
								dir2 = checkState.getValue(LDIR);
								if (dir2 != EnumFacing.UP && dir2 != EnumFacing.DOWN) {
									for (EnumFacing face : EnumFacing.HORIZONTALS) {
			...
			if (world.getBlockState(pos.down()).getBlock() instanceof MistSoil) {
				...
				if (check) {
					for (int i = 0; i < j; ++i) {
						...
						world.setBlockState(checkPos, MistBlocks.R_TREE_BLOCK.getDefaultState().withProperty(MistWoodBlock.AXIS, MistWoodBlock.EnumAxis.fromFacingAxis(face.getAxis())), 2);
	...
	protected ItemStack getBranch() {
		return new ItemStack(MistBlocks.R_TREE_BRANCH);
	}

	@Override
	protected ItemStack getTrunk() {
		return new ItemStack(MistBlocks.R_TREE_BRANCH, 1, 6);
	}

	@Override
	protected ItemStack getBlock() {
		return new ItemStack(MistBlocks.R_TREE_BLOCK);
	}

	@Override
	protected ItemStack getNode() {
		return new ItemStack(MistBlocks.R_TREE_BLOCK, 1, 7);
	}
}
...
public class MistTrunkSnow extends MistTreeTrunk {

	public MistTrunkSnow() {
		super(5.0F, 2, 2, true, true, true, (MistTreeLeaves)MistBlocks.SNOW_LEAVES, 1, 0, 5, new int[] {1, 1, 1, 2});
	}
	...
	protected int getMaxTreeHeight(World world, BlockPos rootPos, int minTrunckLength, long posRand, int soilDepth) {		
		return minTrunckLength + 10 + (int)(posRand % 6) % 3;
	}
	...
	protected int canGrowth(World world, BlockPos pos, IBlockState state, int size, EnumFacing dir, ArrayList<EnumFacing> availableGrowthDirection, boolean isBud, int totalLength,
		...
		if (soil.getValue(IWettable.WET) && humus > 0) {
			...
			if (temp < -0.5 || temp > 1.7 || humi < 5) return 0;
			...
			if (branchLength > 0) {
				...
				if (branchLength < Math.min(trunckSize == 1 && trunckLength > minTrunckLength ? 2 : trunckSize, trunckLength <= minTrunckLength + 1 ? 2 : 3)) return 2;
							...
							if (rand.nextInt(3) > 0) {
								...
								if (checkEnvironment(world, checkPos, false)) {
									...
									if (downState.getBlock() == this ? getDir(downState) != dir : true) {
										...
										int bud = 0;
										for (int j = 0; j < (i == minTrunckLength ? 2 : 3) && j < (size == 1 ? 2 : size); ++j) {
			...
			if (world.getBlockState(pos.down()).getBlock() instanceof MistSoil) {
				...
				if (check) {
					...
					for (int i = 0; i < j; ++i) {
						...
						world.setBlockState(checkPos, MistBlocks.SNOW_BLOCK.getDefaultState().withProperty(MistWoodBlock.AXIS, MistWoodBlock.EnumAxis.fromFacingAxis(face.getAxis())), 2);
						...
						if (i > 1) {
							for (EnumFacing branchFace : FacingHelper.NOTDOWN) {
								if (branchFace.getAxis() != face.getAxis() && rand.nextInt(3) == 0) {
									...
									if (world.getBlockState(branchPos.offset(face.getOpposite())).getBlock() != MistBlocks.SNOW_BRANCH) {
										if (world.getBlockState(branchPos).getMaterial().isReplaceable()) {
											...
											world.setBlockState(branchPos, MistBlocks.SNOW_BRANCH.getDefaultState().withProperty(MistBlockBranch.SIZE, 1).withProperty(MistBlockBranch.AXIS, branchFace.getAxis()), 2);
										}
									}
								}
							}
	...
	protected ItemStack getBranch() {
		return new ItemStack(MistBlocks.SNOW_BRANCH);
	}

	@Override
	protected ItemStack getTrunk() {
		return new ItemStack(MistBlocks.SNOW_BRANCH, 1, 6);
	}

	@Override
	protected ItemStack getBlock() {
		return new ItemStack(MistBlocks.SNOW_BLOCK);
	}

	@Override
	protected ItemStack getNode() {
		return new ItemStack(MistBlocks.SNOW_BLOCK, 1, 7);
	}
}
...
public class RenderMistPainting extends Render<EntityMistPainting> {

	private static final ResourceLocation MIST_PAINTING_TEXTURE = new ResourceLocation("mist:textures/painting/mist_paintings.png");

	public RenderMistPainting(RenderManager renderManager) {
		super(renderManager);
	}

	@Override
	public void doRender(EntityMistPainting entity, double x, double y, double z, float entityYaw, float partialTicks) {
		GlStateManager.pushMatrix();
		GlStateManager.translate(x, y, z);
		GlStateManager.rotate(180.0F - entityYaw, 0.0F, 1.0F, 0.0F);
		GlStateManager.enableRescaleNormal();
		this.bindEntityTexture(entity);
		EntityMistPainting.EnumArt enumart = entity.art;
		float f = 0.0625F;
	...
	@Override
	protected ResourceLocation getEntityTexture(EntityMistPainting entity) {
		return MIST_PAINTING_TEXTURE;
	}

	private void renderPainting(EntityMistPainting painting, int width, int height, int textureU, int textureV) {
		float f = (-width) / 2.0F;
	...
	private void setLightmap(EntityMistPainting painting, float p_77008_2_, float p_77008_3_) {

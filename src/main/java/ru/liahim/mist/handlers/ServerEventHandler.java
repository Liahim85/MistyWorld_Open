...
public class ServerEventHandler {
        ...
        if (event.phase == Phase.START && event.side == Side.SERVER) {
            ...
            if (player != null) {
                ...
                if (!maskSync.containsKey(uuid)) {
                    maskSync.put(uuid, ItemStack.EMPTY);
                }
                ...
                if (
                    maskHandler.isMaskChanged() ||
                    (
                        mask != null &&
                        mask.willAutoSync(stack, player) &&
                    ...
                    try {
                        if (maskHandler.isGlobalChanged()) {
                            PacketHandler.INSTANCE.sendToDimension(
                                new PacketMaskSync(player, stack),
                ...
                if (player.isPlayerSleeping()) {
                    if (world.provider.getDimension() == 0) {
                        if (player.isPlayerSleeping() && player.ticksExisted % 90 == 0) {
                            ...
                            if (
                                otherWorld != null &&
                                !otherWorld.playerEntities.isEmpty() &&
                                !isAllPlayersAsleep(otherWorld)
                            ) {
    ...
    public void onPlayerWakeUp(PlayerWakeUpEvent event) {
        ...
        if (!world.isRemote && world.provider.getDimension() == Mist.getID() && player == world.playerEntities.get(0)) {
            ...
            if (
                normalWorldEmpty && isAllPlayersFullyAsleep(world) && world.getGameRules().getBoolean("doDaylightCycle")
            ) {
    ...
    public void breakSpeed(BreakSpeed event) {
        if (event.getState().getBlock() instanceof IMistStone) {
            ...
            if (
                state.getBlock() instanceof MistTreeTrunk &&
                ((MistTreeTrunk) state.getBlock()).getDir(state) == EnumFacing.UP
            ) {
                event.setNewSpeed(
                    Math.min(
                        event.getOriginalSpeed(),
                        event.getEntityPlayer().getDigSpeed(state, event.getPos().up()) / 8
                    )
                );
            }
    ...
    public void getStone(LivingExperienceDropEvent event) {
        ...
        if (!ModConfig.dimension.disableStoneDrop && event.getAttackingPlayer() != null) {
            ...
            if (!world.isRemote && world.provider.getDimension() != Mist.getID()) {
                if (
                    event.getEntityLiving() instanceof EntityEnderman &&
                    event.getAttackingPlayer() instanceof EntityPlayerMP
                ) {
                    ...
                    if (
                        ...
                        !player
                            .getAdvancements()
                            .getProgress(
                                ((WorldServer) world).getAdvancementManager()
                                    .getAdvancement(new ResourceLocation(Mist.MODID, "story/full_set"))
                            )
                            .isDone()
                    ) {
    ...
    protected void dropStone(World world, EntityEnderman enderman) {
        EntityItem itemPortalDown = new EntityItem(
            world,
            enderman.posX,
            enderman.posY,
            enderman.posZ,
            new ItemStack(MistBlocks.PORTAL_BASE, 1, 2)
        );
        EntityItem itemPortalUp = new EntityItem(
            world,
            enderman.posX,
            enderman.posY,
            enderman.posZ,
            new ItemStack(MistBlocks.PORTAL_BASE, 1, 3)
        );
        ...
        if (!world.isRemote && !Dimension.loadedDimBlackList.contains(world.provider.getDimension())) {
            ...
            if (block == Blocks.GOLD_BLOCK || block == MistBlocks.PORTAL_BASE) {
                ...
                if (block == Blocks.GOLD_BLOCK) {
                    ...
                    if (
                        up.getBlock() == MistBlocks.PORTAL_BASE &&
                        up.getValue(MistPortalStone.ISUP) &&
                        down.getBlock() == MistBlocks.PORTAL_BASE &&
                        !down.getValue(MistPortalStone.ISUP)
                    ) {
                        check = true;
                    }
                } else {
                    boolean isUp = blockState.getValue(MistPortalStone.ISUP);
                    ...
                    if (
                        portalState.getBlock() == Blocks.GOLD_BLOCK &&
                        checkState.getBlock() == MistBlocks.PORTAL_BASE &&
                        checkState.getValue(MistPortalStone.ISUP) == !isUp
                    ) {
                        check = true;
                    }
                }
        ...
        if (!event.getWorld().isRemote) {
            if (event.getPlayer() != null && !event.getPlayer().isCreative()) {
                ...
                //Sapling drop
                    ...
                    event.setCanceled(
                        ((MistTreeTrunk) event.getState().getBlock()).dropSapling(event.getWorld(), event.getPos())
                    );
                }
            }
        }
        ...
        if (item == Items.MAP) {
            ...
            if (player.world.provider.getDimension() == Mist.getID()) {
                ...
                if (player.getPosition().getY() > MistWorld.getFogHight(player.world, 0) - 6) {
                    mistMapUp =
                        ItemMistMapUp.setupNewMap(player.world, player.posX, player.posZ, (byte) 0, true, false);
                } else {
                    mistMapUp =
                        ItemMistMapDown.setupNewMap(player.world, player.posX, player.posZ, (byte) 0, true, false);
                }
                stack.shrink(1);
                if (stack.isEmpty()) {
            ...
            item != MistItems.GLASS_CONTAINER &&
            (item.getItemUseAction(stack) == EnumAction.EAT || item.getItemUseAction(stack) == EnumAction.DRINK)
        ) {
            EntityPlayer player = event.getEntityPlayer();
            ...
            if (event.getEntityPlayer().isSneaking()) {
                if (block == MistBlocks.CAMPFIRE || block == MistBlocks.CAMP_STICK) {
                    ...
                    if (world.getBlockState(pos.up()).getMaterial().isReplaceable()) {
                        ...
                        if (
                            MistRegistry.checkShiftPlacing(
                                world,
                                pos.up(),
                                stack,
                                ...
                                world.getBlockState(pos).getBlockFaceShape(world, pos, EnumFacing.UP)
                            )
                        ) {
                            event.setCanceled(true);
                            event.setCancellationResult(EnumActionResult.SUCCESS);
                    ...
                    if (MistFlowerPot.canBePotted(stack)) {
                        ...
                        if (te != null) {
                            if (event.getEntityPlayer().capabilities.isCreativeMode) {
                                ...
                                te.setItemStack(stack.splitStack(1));
                            }
                            te.markDirty();
        ...
        if (!event.getWorld().isRemote && !event.getEntityPlayer().isCreative()) {
            ...
            if (state.getBlock() instanceof BlockCrops) {
                if (event.getEntityPlayer() != null) {
                    setMulchDelay(event.getEntityPlayer().getUniqueID(), 5);
                }
            } else if (
                state.getBlock() instanceof IFarmland &&
                state.getValue(IFarmland.MULCH) &&
                event.getHitVec().y - event.getPos().getY() > 0.9375F &&
                !isMulchDelay(event.getEntityPlayer().getUniqueID())
            ) {
    ...
    public void attachCapabilitiesPlayer(AttachCapabilitiesEvent<Entity> event) {
        if (event.getObject() instanceof EntityPlayer) {
            event.addCapability(
                new ResourceLocation(Mist.MODID, "container"),
                new MistCapability.Provider(new MistCapaHandler())
            );
        ...
        if (!event.getWorld().isRemote) {
            if (event.getEntity() instanceof EntityPlayerMP) {
                ...
                if (event.getWorld().provider.getDimension() == Mist.getID()) {
                    PacketHandler.INSTANCE.sendTo(new PacketSeedSync(event.getWorld().getSeed()), playerMP);
                }
                ...
                PacketHandler.INSTANCE.sendTo(
                    new PacketSkillSync(ISkillCapaHandler.getHandler(playerMP).getSkillsArray()),
                    playerMP
                );
                IFoodHandler foodHandler = IFoodHandler.getHandler(playerMP);
                PacketHandler.INSTANCE.sendTo(
                    new PacketMushroomSync(foodHandler.getMushroomList(false), false),
                    playerMP
                );
                PacketHandler.INSTANCE.sendTo(
                    new PacketMushroomSync(foodHandler.getMushroomList(true), true),
                    playerMP
                );
                ...
                PacketHandler.INSTANCE.sendTo(
                    new PacketToxicSync(capa.getPollution(), HurtType.POLLUTION.getID()),
                    playerMP
                );
                ...
                if (event.getWorld().provider.getDimension() == Mist.getID()) {
                    ...
                    if (
                        res != null &&
                        MistRegistry.mobsDimsBlackList.contains(res.getResourceDomain()) ||
                        MistRegistry.mobsBlackList.contains(res)
                    ) {
                        event.setCanceled(true);
                    }
                }
    ...
    public void cloneCapabilitiesEvent(PlayerEvent.Clone event) {
        try {
            ...
            Mist.logger.error(
                "Could not clone player [" + event.getOriginal().getName() + "] mask when changing dimensions"
            );
        }
        try {
            ...
            Mist.logger.error(
                "Could not clone player [" +
                event.getOriginal().getName() +
                "] food study list when changing dimensions"
            );
        }
        try {
            ...
            Mist.logger.error(
                "Could not clone player [" + event.getOriginal().getName() + "] skills list when changing dimensions"
            );
        }
    }
                                ...
                                if (slot.getSlotType() == EntityEquipmentSlot.Type.ARMOR) {
                                    ...
                                    if (armor.getItem() instanceof ItemArmor && !IMask.isMask(armor)) {
                                        ...
                                        if (
                                            ...
                                            material != ModItems.NIOBIUM_ARMOR &&
                                            material != ModItems.RUBBER_MATERIAL
                                        ) {
                                ...
                                if (slot.getSlotType() == EntityEquipmentSlot.Type.ARMOR) {
                                    ...
                                    if (armor.getItem() instanceof ISuit) {
                                        ...
                                        if (tag != null) {
                                            ...
                                            if (suit.getCount() == 0) {
                                                armor.removeSubCompound(MistTags.nbtInnerSuitTag);
                                            } else {
                        ...
                        if (player.getRNG().nextBoolean()) {
                            ...
                            if (IMask.isMask(mask)) {
                                ...
                                if (drop && !player.world.isRemote) {
                                    ...
                                    if (filter.isItemStackDamageable()) {
                                        player.entityDropItem(filter, 1);
                                    }
                                }
                            }
                        }
    ...
    public static void setSpawnPos(EntityPlayerMP player) {
        ...
        if (pos != null) {
            state = DimensionManager.getWorld(Mist.getID()).getBlockState(pos);
        }
    ...
    public void entityItemDead(ItemExpireEvent event) {
        ...
        if (item == MistItems.ROCKS || (item == Items.FLINT && world.provider.getDimension() == Mist.getID())) {
            ...
            if (
                world.getBlockState(pos).getMaterial().isReplaceable() && world.isSideSolid(pos.down(), EnumFacing.UP)
            ) {
                if (item == MistItems.ROCKS) {
                    ...
                    world.setBlockState(
                        pos,
                        MistBlocks.LOOSE_ROCK.getDefaultState().withProperty(MistLooseRock.TYPE, 1)
                    );
                }
            }
    ...
    public void onMobsDeath(LivingDeathEvent event) {
        ...
        if (entity instanceof EntityPlayer && !((EntityPlayer) entity).capabilities.isCreativeMode) {
            ...
            if (mob instanceof EntityLiving && !(mob instanceof IMob)) {
                ...
                if (res != null) {
                    int point = 0;
                    if (MistRegistry.dimsForSkill.containsKey(res.getResourceDomain())) {
                        point = MistRegistry.dimsForSkill.get(res.getResourceDomain());
                    }
                    if (MistRegistry.mobsForSkill.containsKey(res)) {
                        point = MistRegistry.mobsForSkill.get(res);
                    }
                    if (point > 0) {
                        ISkillCapaHandler.getHandler((EntityPlayer) entity).addSkill(Skill.CUTTING, point);
                    }
                }
            }
        }
    ...
    public void getXPOrb(PlayerPickupXpEvent event) {
        if (!event.getEntityPlayer().world.isRemote) {
            ...
            if (
                !mask.isEmpty() &&
                mask.isItemDamaged() &&
                EnchantmentHelper.getEnchantmentLevel(Enchantments.MENDING, mask) > 0
            ) {
                ...
                event
                    ...
                    .world.playSound(
                        ...
                        (
                            (
                                event.getEntityPlayer().getRNG().nextFloat() -
                                event.getEntityPlayer().getRNG().nextFloat()
                            ) *
                            0.7F +
                            1.8F
    ...
    private int getBurnTime(ItemStack fuel) {
        ...
        if (item instanceof ItemBlock && Block.getBlockFromItem(item) != Blocks.AIR) {
            ...
            if (block instanceof MistBlockStairs && block.getDefaultState().getMaterial() == Material.WOOD) {
                return 450;
            }
            if (block instanceof MistBlockSlab && block.getDefaultState().getMaterial() == Material.WOOD) {
                return 300;
            }
            if (block instanceof MistBlockStep && block.getDefaultState().getMaterial() == Material.WOOD) {
                return 150;
            }
            ...
            if (block == MistBlocks.FILTER_COAL_BLOCK) {
                return (16 - fuel.getItemDamage()) * 1000;
            }
            if (block == MistBlocks.BIO_SHALE_BLOCK) {
                return 16000;
            }
            if (block == MistBlocks.PEAT && fuel.getItemDamage() == 1) {
                return 2400;
            }
            if (block == MistBlocks.SAPROPEL && fuel.getItemDamage() == 1) {
                return 1200;
            }
        ...
        if (item == MistItems.FILTER_COAL) {
            return Math.max(1, 1600 * (fuel.getMaxDamage() - fuel.getItemDamage()) / fuel.getMaxDamage());
        }
    ...
    public void harvestCheck(HarvestCheck event) {
        if (event.getTargetBlock().getBlock() instanceof MistChest) {

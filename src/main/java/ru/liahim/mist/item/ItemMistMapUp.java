...
public class ItemMistMapUp extends ItemMistMap {
	...
	public void addInformation(ItemStack stack, @Nullable World world, List<String> tooltip, ITooltipFlag flagIn) {
		tooltip.add(I18n.format("item.mist.map_up.tooltip"));
	}
	...
	public static ItemStack setupNewMap(World worldIn, double worldX, double worldZ, byte scale, boolean trackingPosition, boolean unlimitedTracking) {
		ItemStack itemstack = new ItemStack(MistItems.MAP_UP, 1, worldIn.getUniqueDataId("map"));
...
// ---------------------------------------> Mist hook
											if (mist) {
												if (k4 < fogHight) multiset.add(MapColor.QUARTZ);
												else if (k4 < fogHight + 4) multiset.add(MapColor.SILVER);
